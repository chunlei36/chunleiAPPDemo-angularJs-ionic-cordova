...
.controller('HomeCtrl', function($scope, goods, $timeout, $ionicLoading) {
	...
	.controller('homeDetail', function($scope, $stateParams, goods, allgoodShop, $timeout, $ionicLoading, $ionicHistory) {
...
.controller('NearDetailCtrl', function($scope, $stateParams, Content, goods) {
	...
	.controller('MyAccount', function($scope, $http) {
		$scope.latepages = function() {
			history.go(-1)
		}
		$http.jsonp('http://localhost:3000/deliuser/ploginall?callback=JSON_CALLBACK').success(function(data) {
			var users = data
					...
					if(reg_user.length != 11) {
						...
						if(pwd_us1.length >= 6) {
							if(pwd_us1 == pwd_us2) {
								...
								$http.jsonp('http://localhost:3000/deliuser/preg?callback=JSON_CALLBACK', {
									params: usermsg
...
.controller('MeCtrl', function($scope, $ionicPopup) {
...
.controller('MeOrderCtrl', function($scope) {
	...
	//设置
	.controller('MeSetCtrl', function($scope, $ionicPopup) {
		// 触发一个按钮点击，或一些其他目标
	...
	.controller('RegoodsCtrl', function($scope, $ionicPopup) {
		...
		$scope.tktj = function() {
			$scope.data = {}
				// 一个精心制作的自定义弹窗
				//				<ul> <li > < img src = "img/meImages/set_clear.png"/ > < /li> <li > 提交成功! < /li> </ul>
